...
Book actions → Privacy offers Normal, Locked and Hidden. Library → View → Hidden books opens the authenticated hidden collection. Settings → Privacy manages the device passcode, optional iOS biometrics, automatic locking and the background cover. Privacy policies are local to the device/browser profile and hosted account; they are not synced or included in portable backups. EPUBs, server copies and exports remain unencrypted. This protects access through Quire's reader UI, not files accessed outside it or server-administrator access.

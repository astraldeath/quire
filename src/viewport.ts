...
export function layoutViewport(previous:{width:number;height:number},next:{width:number;height:number},editing:boolean){
 if(editing&&Math.abs(previous.width-next.width)<2&&next.height<previous.height)return previous;
 return next;
}
export function installViewport(){
 ...
 const update=()=>{
  ...
  size=layoutViewport(size,{width:window.innerWidth,height:window.innerHeight},editing);

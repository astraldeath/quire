...
export function HostedApp(){
 ...
 useEffect(()=>{if(location.hash.startsWith('#invite='))history.replaceState(null,'',location.pathname);void (async()=>{const saved=await restoreBrowserAccount();if(saved){try{await openAccount(saved);return;}catch{clearBrowserSession();}}const v=await publicRequest('/v1/setup');setSetup(v.required);})().catch(e=>setError(e.message));},[]);

...
const hosted=(origin:string)=>import.meta.env.VITE_HOSTED==='true'&&!isTauri()&&origin===location.origin;
const credentials=(origin:string):RequestCredentials=>hosted(origin)?'same-origin':'omit';
function requestHeaders(token?:string):Record<string,string>{return token?.startsWith('cookie:')?{'X-Quire-Session':token.slice(7)}:token?{Authorization:`Bearer ${token}`}:{}};
export async function restoreBrowserAccount():Promise<Account|undefined>{
 clearBrowserSession();if(!hosted(location.origin))return;
 const r=await fetch(location.origin+'/v1/browser/session',{credentials:'same-origin',redirect:'error',signal:AbortSignal.timeout(30000)});
 if(r.status===401)return;
 if(!r.ok)throw new Error('Unable to restore your session. Refresh to try again.');
 const value=await r.json();if(typeof value?.username!=='string'||typeof value?.sessionId!=='string')throw new Error('Invalid browser session response.');
 return {origin:location.origin,username:value.username,sessionId:value.sessionId};
}
...
export async function login(origin:string,username:string,password:string){
 ...
 const v=await web(origin,hosted(origin)?'/v1/browser/session':'/v1/sessions',{username,password,deviceName:'Quire browser'});
 if(!v?.session?.id||(!hosted(origin)&&typeof v?.token!=='string'))throw new Error('Invalid session response.');
 if(hosted(origin))clearBrowserSession();else tokens.set(`${username}@${origin}`,v.token);
 return v.session as {id:string};
}
export async function logout(a:Account){if(isTauri())return invoke<void>('sync_logout',{server:a.origin,username:a.username,session:a.sessionId});if(hosted(a.origin)){await web(a.origin,'/v1/browser/session',undefined,webToken(a),'DELETE');clearBrowserSession();return;}const key=`${a.username}@${a.origin}`,token=tokens.get(key);tokens.delete(key);if(import.meta.env.VITE_HOSTED==='true')clearBrowserSession();if(token)await web(a.origin,`/v1/sessions/${encodeURIComponent(a.sessionId)}`,undefined,token,'DELETE');}
export async function call(a:Account,body:unknown):Promise<SyncResponse>{
 if(isTauri())return await invoke<SyncResponse>('sync_call',{server:a.origin,username:a.username,body});return web(a.origin,'/v1/sync',body,webToken(a));
}
export interface ServerFile {bookId:string;size:number;uploaded:boolean;watched:boolean}
function webToken(a:Account){if(hosted(a.origin))return `cookie:${a.sessionId}`;const token=tokens.get(`${a.username}@${a.origin}`);if(!token)throw new Error('Sign in again.');return token;}
...
export async function upload(a:Account,book:string,bytes:Uint8Array){
 ...
 const r=await fetch(`${a.origin}/v1/books/${book}/file`,{method:'PUT',redirect:'error',credentials:credentials(a.origin),headers:{...requestHeaders(webToken(a)),'Content-Type':'application/epub+zip'},body:bytes.slice().buffer,signal:AbortSignal.timeout(300000)});if(!r.ok)throw new Error('Upload failed. Your local book is unchanged.');
}
export async function download(a:Account,book:string):Promise<Uint8Array>{
 ...
 const r=await fetch(`${a.origin}/v1/books/${book}/file`,{redirect:'error',credentials:credentials(a.origin),headers:{...requestHeaders(webToken(a))},signal:AbortSignal.timeout(300000)});if(!r.ok||!r.body)throw new Error('The server EPUB is unavailable.');const reader=r.body.getReader();let size=0;const parts:Uint8Array[]=[];while(true){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>128*1024*1024){await reader.cancel();throw new Error('EPUB is too large.');}parts.push(value);}const bytes=new Uint8Array(size);let at=0;for(const p of parts){bytes.set(p,at);at+=p.length;}return bytes;
}
...
export async function uploadShared(a:Account,library:string,id:string,bytes:ArrayBuffer){const r=await fetch(`${a.origin}/v1/admin/libraries/${library}/books/${id}`,{method:'PUT',credentials:credentials(a.origin),headers:{...requestHeaders(webToken(a)),'Content-Type':'application/epub+zip'},body:bytes,signal:AbortSignal.timeout(300000)});if(!r.ok)throw new Error('Could not upload the shared book.');}

export async function downloadServerBackup(a:Account,onProgress:(bytes:number,total:number)=>void):Promise<Blob>{
 const r=await fetch(a.origin+'/v1/admin/backup',{method:'POST',redirect:'error',credentials:credentials(a.origin),headers:{...requestHeaders(webToken(a))},signal:AbortSignal.timeout(300000)});

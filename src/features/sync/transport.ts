...
async function web(origin:string,path:string,body?:unknown,token?:string,method=body?'POST':'GET'){
 const response=await fetch(origin+path,{method,redirect:'error',credentials:'omit',headers:{...(body?{'Content-Type':'application/json'}:{}),...(token?{Authorization:`Bearer ${token}`}:{})},body:body?JSON.stringify(body):undefined,signal:AbortSignal.timeout(path.startsWith('/v1/admin/watches')&&method==='POST'?300000:30000)});

...
export function ensureBookFile(id:string):Promise<Uint8Array>{
 ...
 const task=(async()=>{
  ...
  if(bytes.byteLength>128*1024*1024)throw new Error('EPUB is too large (128 MB maximum).');
  ...
  if(hash!==id)throw new Error('Downloaded EPUB does not match this book.');
  const book=(await listBooks()).find(b=>b.id===id);if(!book)throw new Error('This book is no longer in your library.');

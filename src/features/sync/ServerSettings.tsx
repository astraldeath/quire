...
import {Cloud,RefreshCw,LogOut,Check,ChevronDown,ArrowRight,Search,LoaderCircle,CloudDownload,BookOpen} from 'lucide-react';
...
export function ServerSettings(){
 ...
 const reconnect=()=>perform(async()=>{setAccount(state.account?.username??'');setURL(state.account?.origin??'');await disconnect();});
 ...
 <div><h3><Cloud aria-hidden="true"/> Your server</h3><p className="muted">Keep your library, reading progress, and saved passages in sync.</p></div>
 {state.enabled&&state.account?<>
 <section className="server-card">
 <div className="server-account"><span className="server-account-icon"><Cloud aria-hidden="true"/></span><div><strong>{state.account.username}</strong><span className="muted">{state.account.origin}</span></div><span className="server-connected">{needsLogin?'Session expired':'Connected'}</span></div>
 <div className="server-sync-status" role="status">{status.busy?<LoaderCircle className="server-spinner" aria-hidden="true"/>:<RefreshCw aria-hidden="true"/>}<div><strong>{status.message}</strong><span className="muted">{state.lastSync?`Last synced ${new Date(state.lastSync).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}`:'Ready to sync'}{state.pending.length>0?` · ${state.pending.length} pending changes`:''}</span></div></div>
 <button className="primary server-connect" disabled={busy||status.busy} onClick={()=>void (needsLogin?reconnect():perform(syncNow))}><RefreshCw aria-hidden="true"/>{needsLogin?'Sign in again':status.busy?'Syncing…':'Sync now'}</button>
 </section>
 <section className="server-library-info"><h3>Your library, on every device</h3><div><CloudDownload aria-hidden="true"/><p>Covers and book details sync automatically.</p></div><div><BookOpen aria-hidden="true"/><p>Open a book to download it. Downloaded books stay available offline.</p></div></section>
 <section className="server-disconnect"><div><h3>This device</h3><p className="settings-note">Signing out keeps downloaded books and unsynced changes.</p></div><button className="server-secondary" disabled={busy||status.busy} onClick={()=>void perform(disconnect)}><LogOut aria-hidden="true"/>Sign out</button></section>

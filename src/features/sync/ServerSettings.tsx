...
import {Cloud,RefreshCw,LogOut,Check} from 'lucide-react';
...
export function ServerSettings(){
 ...
 <details><summary>Advanced server address</summary><label>Server URL<input type="url" placeholder="https://books.example.com:8443" value={url} disabled={busy} onChange={e=>{setURL(e.target.value);setServer(null);}}/></label></details>
 ...
 <button className="text-action" disabled={busy}>{busy?'Connecting…':server?'Sign in and sync':'Find server'}</button>
 {!isTauri()&&<p className="settings-note">Browser sessions last until this tab closes. The server must allow this browser’s origin. Installed apps keep sessions in secure device storage.</p>}
 </form>}

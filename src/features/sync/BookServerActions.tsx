...
export function BookServerActions({book}:{book:Book}){
 ...
 useEffect(()=>{setLoaded(false);void loadSync().then(async s=>{if(s.enabled&&s.account){setAccount(s.account);setRemote((await files(s.account)).find(f=>f.bookId===book.id));}}).catch(e=>setError(String(e))).finally(()=>setLoaded(true));},[book.id]);
 if(!account)return null;
 if(!loaded)return <p className="muted" role="status">Checking server copy…</p>;
 ...
 {book.local&&!remote&&<button type="button" className="text-action" disabled={!!busy} onClick={()=>void run('Uploading',async()=>{const bytes=await getFile(book.id);if(!bytes)throw new Error('Local EPUB is unavailable.');await syncNow();await upload(account,book.id,bytes);})}><CloudUpload/>Upload EPUB</button>}

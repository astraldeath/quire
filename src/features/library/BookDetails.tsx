...
export function BookDetails({ book, onClose, onSave, onRemove, onRead, onImport, onDelete }: {
  ...
  return <Modal title="Book details" onClose={onClose}><form onSubmit={e => { e.preventDefault(); void run(() => onSave({ ...draft, title: draft.title.trim(), author: draft.author.trim(), series: draft.series.trim() })); }}>
    ...
    {!book.local&&<button type="button" className="text-action" onClick={onImport}><HardDriveDownload/>Import EPUB instead</button>}

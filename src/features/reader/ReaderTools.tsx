...
import { lookupDefinition, type DefinitionGroup, wiktionaryUrl } from './definitions';
...
export function ReaderTools({otherPanelOpen,onOpen,toolbar,view,book,visible,onSave,navigate}: Props) {
  const [selection,setSelection] = useState<Selection|null>(null);
  const [panel,setPanel] = useState<'saved'|'note'|'define'|'search'|null>(null);
  ...
  useEffect(()=>{
    ...
    setDefining(true);setDefinitions([]);setDefinitionError('');
    const timeout=setTimeout(()=>controller.abort(),15000);
    void lookupDefinition(query,controller.signal).then(result=>{if(active)setDefinitions(result);}).catch(error=>{
      if(active)setDefinitionError(controller.signal.aborted?'Wiktionary took too long to respond. Please try again.':error instanceof Error?error.message:'Could not load this definition.');
    }).finally(()=>{clearTimeout(timeout);if(active)setDefining(false);});
  ...
  useEffect(() => {
    ...
    const install = ({doc,index}:{doc:Document;index:number}) => {
      if (installed.has(doc)) return; installed.add(doc);
      let timer: ReturnType<typeof setTimeout>;
      const changed = () => { clearTimeout(timer); timer=setTimeout(()=>{
        const selected=doc.getSelection();
        if (!selected || selected.isCollapsed || !selected.rangeCount || !selected.toString().trim()) { setSelection(null); return; }
        try { setSelection({cfi:view.getCFI(index,selected.getRangeAt(0)),text:selected.toString().trim(),doc}); } catch { setMessage('This selection could not be saved.'); }
      },180); };
      doc.addEventListener('selectionchange',changed);
      disposers.push(()=>{clearTimeout(timer);doc.removeEventListener('selectionchange',changed);});
    };
  ...
  const bookmark=async()=>{
    ...
    const previous=current.current.book.annotations??[];
    const existing=previous.find(a=>a.kind==='bookmark'&&a.cfi===position.cfi);
  ...
  const search=async(term:string)=>{
    ...
    try{for await(const batch of view.search({query:term})){if(id!==searchId.current){view.clearSearch();break;}if(typeof batch!=='string'&&batch.subitems)setResults(old=>[...old,...batch.subitems!.map(r=>({cfi:r.cfi,text:r.excerpt.pre+r.excerpt.match+r.excerpt.post}))]);}}
    catch{if(id===searchId.current)setMessage('Could not search this book.');}finally{if(id===searchId.current)setSearching(false);}
    });searchWork.current=work;await work;
  };
  ...
  return <>
    ...
    {selection&&!panel&&<div className="selection-tools" role="toolbar" aria-label="Selected text actions" onPointerDown={e=>e.preventDefault()}>
      ...
      <button title="Add note" aria-label="Add note" onClick={()=>{setEditing({id:'',kind:'highlight',cfi:selection.cfi,text:selection.text,note:'',section:book.position?.section??'',createdAt:0,updatedAt:0});setNote('');setPanel('note');}}><Pencil /></button>
      <button title="Define (English, online)" aria-label="Define selected text" onClick={()=>void define()}><BookOpen /></button>
    ...
    {panel&&<ReaderDialog onClose={close} label={panel==='saved'?'Bookmarks and highlights':panel==='note'?'Edit note':panel==='define'?'Definition':'Search in book'}>
      ...
      {panel==='define'&&<div className="dictionary-entry"><h3>{query}</h3>
        {defining&&<p role="status">Looking up definition…</p>}
        {definitions.map((group,index)=><section key={index}><h4>{group.partOfSpeech}</h4><ol>{group.senses.map((sense,i)=><li key={i}>{sense}</li>)}</ol></section>)}
        {definitionError&&<div><p role="status">{definitionError}</p><button onClick={()=>setLookupAttempt(n=>n+1)}>Try again</button></div>}
        <small>Definitions adapted from <a href={dictionaryUrl} target="_blank" rel="noreferrer">Wiktionary</a>, <a href="https://creativecommons.org/licenses/by-sa/4.0/" target="_blank" rel="noreferrer">CC BY-SA 4.0</a>.</small>
      </div>}

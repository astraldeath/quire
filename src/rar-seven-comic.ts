...
function decoderSession(bytes: Uint8Array) {
  ...
  function bounded<T>(operation: () => Promise<T>): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      ...
      Promise.resolve()
        .then(() => {
          if (stopped) throw new Error('Comic decoder was closed.');
          return operation();
        })
        .then((value) => {
          cleanup();
  ...
  const ready = (async () => {
    let initializedCallback!: () => void;
    const initialized = new Promise<void>((resolve) => {
      initializedCallback = resolve;
    });
    const Remote = wrap<new (ready: () => void) => Decoder>(worker);
    const decoder = await bounded(() => new Remote(proxy(initializedCallback)));
    await bounded(() => initialized);
    await bounded(() =>
      decoder.open(new Blob([bytes as Uint8Array<ArrayBuffer>])),
    );
    return decoder;
  })().catch((error) => {
    close();
...
export async function openRarSevenComic(
  ...
  let session = decoderSession(bytes);
  try {
    const decoder = await session.ready;
    const pages = indexComicEntries(
      await session.bounded(() => decoder.listFiles()),
    );
    return openDecodedComic(
      pages,
      async (name, signal) => {
        signal?.throwIfAborted();
        // Native extraction is synchronous inside the worker. Termination is
        // the only prompt cancellation; the next queued page opens fresh state.
        if (session.stopped) session = decoderSession(bytes);
        const current = session;
        const abort = () => current.close();
        signal?.addEventListener('abort', abort, { once: true });
        try {
          const decoder = await current.ready;
          const result = await current.bounded(() =>
            decoder.extractSingleFile(name),
          );
          if (!result?.fileData)

...
describe('durable library lifecycle', () => {
  it('retains organization and resume data after removal and identical reimport', async () => {
    ...
    expect((await listBooks()).find((b) => b.id === book.id)).toEqual({
      ...edited,
      local: false,
    });
    // A late metadata save cannot recreate local availability.
    await saveBook(edited);
    expect((await listBooks()).find((b) => b.id === book.id)?.local).toBe(
      false,
    );
    ...
    expect(
      (await independentConnection.get('books', book.id)).metadata.annotations,
    ).toEqual(edited.annotations);
  ...
  it('merges new defaults into persisted partial reader preferences', async () => {
    ...
    expect(await loadPreferences()).toEqual({
      ...defaults,
      theme: 'light',
      reader: { ...defaults.reader, size: 25 },
    });
...
it('browser progress updates preserve saved notes across an independent connection', async () => {
  ...
  await saveBookAnnotations(book.id, notes);
  await saveReadingPosition(book.id, {
    cfi: 'epubcfi(/6/4)',
    fraction: 0.4,
    section: 'two',
    updatedAt: 1,
  ...
  const connection = await openDB('quire-library');
  expect((await connection.get('books', book.id)).metadata.annotations).toEqual(
    notes,
  );
  connection.close();
});
...
it('restores a batch while retaining existing files for data-only records', async () => {
  ...
  expect(Array.from((await s.getFile(book.id))!)).toEqual([4]);
  expect((await s.listBooks()).find((b) => b.id === book.id)?.title).toBe(
    'After',
  );
  expect(Array.from((await s.getFile('restore-new'))!)).toEqual([5]);
});
...
it('removes selected books and saved data without touching other books', async () => {
  ...
  expect(
    (await s.listBooks()).find((x) => x.id === 'keep-me')?.annotations,
  ).toEqual(b.annotations);
});

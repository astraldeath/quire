// Foliate-js MIT; local adapter sanitizes before parsing and rejects DRM.
...
const decompressPalmDOC = (array) => {
  ...
  for (let i = 0; i < array.length; i++) {
    if (output.length > 65536) throw new Error('MOBI text record is too large');
    const byte = array[i];
    if (byte === 0)
...
const huffcdic = async (mobi, loadRecord) => {
  ...
  const decompress = (byteArray, depth = 0) => {
    if (depth > 32) throw new Error('Invalid recursive MOBI dictionary');
    ...
    for (let i = 0; i < bitLength;) {
      ...
      if (!codeLength || codeLength > 32)
        throw new Error('Invalid MOBI code length');
      ...
      if (!decompressed) {
        ...
        expandedDictionarySize += result.length;
        if (expandedDictionarySize > 24 * 1024 * 1024)
          throw new Error('MOBI dictionary is too large');
        dictionary[code] = [result, true];
      }
      if (output.length + result.length > 65536)
        throw new Error('MOBI text record is too large');
...
export class MOBI extends PDB {
  ...
  #getHeaders(buf) {
    ...
    if (palmdoc.encryption)
      throw new Error(
        'DRM-protected Kindle books are unsupported. Import a DRM-free edition.',
      );
    if (palmdoc.numTextRecords > 4096 || palmdoc.recordSize > 65536)
      throw new Error('MOBI text is too large');

...
interface LibraryDB extends DBSchema {books:{key:string;value:Record};preferences:{key:string;value:Preferences|SyncState};files:{key:string;value:Uint8Array}}
...
const browser=():Promise<IDBPDatabase<LibraryDB>>=>new Promise((resolve,reject)=>{let blocked=false;const message='Reload your other Quire tabs, then reload this tab to finish the library storage upgrade.';const timeout=setTimeout(()=>{blocked=true;reject(new Error(message));},15000);void openDB<LibraryDB>(browserName,2,{async upgrade(db,oldVersion,_newVersion,tx){
 if(oldVersion<1){db.createObjectStore('books',{keyPath:'id'});db.createObjectStore('preferences');}
 if(oldVersion<2){const files=db.createObjectStore('files');let cursor=await tx.objectStore('books').openCursor();while(cursor){const record=cursor.value;if(record.file!==undefined)await files.put(record.file,record.id);await cursor.update({id:record.id,metadata:{...record.metadata,local:record.file!==undefined}});cursor=await cursor.continue();}}
},blocked(){blocked=true;clearTimeout(timeout);reject(new Error(message));},blocking(){void browserPromise?.then(db=>db.close());window.dispatchEvent(new Event('quire-database-upgraded'));}}).then(db=>{clearTimeout(timeout);if(blocked)db.close();else resolve(db);},error=>{clearTimeout(timeout);reject(error);});});
...
export async function listBooks():Promise<Book[]>{
 ...
 return(await(await idb()).getAll('books')).map(r=>({...r.metadata,local:r.metadata.local}));
}
...
async function commit(sync:SyncState,writes:Write[]=[],deleted:string[]=[]){
 if(isTauri()){
  ...
  const tx=(await idb()).transaction(['books','files','preferences'],'readwrite');
  try{for(const id of deleted){await tx.objectStore('books').delete(id);await tx.objectStore('files').delete(id);}for(const w of writes){const old=await tx.objectStore('books').get(w.book.id);if(w.fileMode==='set')await tx.objectStore('files').put(w.file!,w.book.id);else if(w.fileMode==='remove')await tx.objectStore('files').delete(w.book.id);await tx.objectStore('books').put({id:w.book.id,metadata:{...w.book,local:w.fileMode==='set'||w.fileMode==='keep'&&!!old?.metadata.local}});}await tx.objectStore('preferences').put(sync,'sync');await tx.done;}
...
export const markBooksRead=(ids:string[],finished:boolean)=>edit(books=>({writes:books.filter(b=>ids.includes(b.id)).map(book=>{const next={...book};if(finished)next.position={cfi:book.position?.cfi??'',section:book.position?.section??'',fraction:1,updatedAt:Date.now()};else delete next.position;return {book:next,fileMode:'keep' as const};})}));

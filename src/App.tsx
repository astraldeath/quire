...
export function App({accountActions,serverLibraries=[],onImport}:{accountActions?:ReactNode;serverLibraries?:{id:string;name:string;bookIds:string[]}[];onImport?:(id:string,bytes:Uint8Array)=>Promise<void>}={}) {
 const [libraryScope,setLibraryScope]=useState('all');
  ...
  const scopedBooks=libraryScope==='all'?books:books.filter(b=>libraryScope==='personal'?!serverLibraries.some(l=>l.bookIds.includes(b.id)):serverLibraries.find(l=>l.id===libraryScope)?.bookIds.includes(b.id));
  const entries = entriesFor(scopedBooks, preferences, query, reading, group);
  ...
  return <>
    ...
    {opened ? <Reader book={books.find(b => b.id === opened.book.id) ?? opened.book} onAnnotations={saveAnnotations} bytes={opened.bytes} preferences={preferences.reader} onPreferences={reader => changePreferences({ ...preferencesRef.current, reader })} onPosition={savePosition} onClose={() => setOpened(null)} /> : <div className="library-shell">
      <header className="topbar"><button className="wordmark" aria-label="Quire library" onClick={() => goLibrary()}>quire<span>.</span></button><nav className="sections" aria-label="Library sections"><button className={!reading ? 'selected' : ''} onClick={() => goLibrary()}>Library</button><button className={reading ? 'selected' : ''} onClick={() => goLibrary(true)}>Reading</button></nav>
        ...
        <button className="add-button" aria-label="Add books" disabled={!!busy || loading} onClick={() => input.current?.click()}><Plus /><span>Add books</span></button><button className="icon" aria-label="Settings" onClick={() => setSettings(true)}><Settings2 /></button>{accountActions}
      </header>

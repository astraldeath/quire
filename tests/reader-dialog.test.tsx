...
import { ReaderDialog } from '../src/features/reader/ReaderDialog';
(
...
it('tracks the visual viewport when the software keyboard shrinks and pans it', async () => {
  ...
  try {
    await act(async () =>
      root.render(
        <ReaderDialog label="Note" onClose={() => {}}>
          <textarea defaultValue="Draft" />
        </ReaderDialog>,
      ),
    );
    const layer = host.querySelector<HTMLElement>('.reader-dialog-layer')!;
    ...
    expect(layer.style.height).toBe('410px');
    expect(layer.style.top).toBe('74px');
...
it('dismisses from the backdrop without treating inside clicks as outside', async () => {
  ...
  await act(async () =>
    root.render(
      <ReaderDialog label="Reading settings" onClose={() => closed++}>
  ...
  await act(async () =>
    host.querySelector<HTMLElement>('.reader-dialog-layer')!.click(),
  );

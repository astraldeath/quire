...
it('accepts locationless manual completion but requires locations for partial progress and annotations',()=>{
 const response=(kind:string,value:unknown)=>({results:[],changes:[{bookId:'a'.repeat(64),kind,recordId:'default',revision:1,cursor:1,candidates:[{operationId:'op',createdAt:1,deleted:false,value}]}],cursor:1,hasMore:false});
 const completed=response('position',{cfi:'',fraction:1,section:''});
 expect(validateResponse(completed,0,[])).toEqual(completed);
 for(const fraction of [0,.5,.999])expect(()=>validateResponse(response('position',{cfi:'',fraction}),0,[])).toThrow();
 expect(()=>validateResponse(response('annotation',{kind:'highlight',cfi:'',text:'word'}),0,[])).toThrow();
});

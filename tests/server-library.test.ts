...
const book={id:createHash('sha256').update(new Uint8Array([1,2,3])).digest('hex'),title:'My title',author:'Author',series:'',volume:null,cover:'',local:false,addedAt:1,annotations:[{id:'note',kind:'highlight' as const,cfi:'cfi',text:'word',note:'Keep me',section:'one',createdAt:1,updatedAt:1}]};
...
it('downloads once on opening and preserves saved notes and custom metadata',async()=>{
 ...
 expect(download).toHaveBeenCalledTimes(1);expect(importEpub).not.toHaveBeenCalled();expect((await listBooks())[0]).toMatchObject({title:'My title',annotations:book.annotations,local:true});
});

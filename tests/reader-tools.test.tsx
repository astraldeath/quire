...
async function setup(annotations: Book['annotations'] = []) {
  ...
  text.textContent = 'A passage worth remembering';
  document.body.append(toolbar, host, text);
  const view = Object.assign(document.createElement('div'), {
    renderer: { getContents: () => [{ doc: document, index: 0 }] },
    getCFI: () => 'epubcfi(/6/2!/4/2:0)',
...
it('bookmarks the current persisted reading location', async () => {
  const ctx = await setup();
  await click('Bookmarks and highlights');
  expect(ctx.onSave).not.toHaveBeenCalled();
  const add = Array.from(document.querySelectorAll('button')).find(
    (b) => b.textContent === 'Bookmark this page',
  )!;
  await act(async () => add.click());
  expect(ctx.onSave).toHaveBeenCalledWith([
    expect.objectContaining({
      kind: 'bookmark',
      cfi: ctx.book.position!.cfi,
      section: 'Chapter one',
    }),
  ]);
...
it('retains selected passage when selection collapses after opening the note editor', async () => {
  ...
  expect(ctx.onSave).toHaveBeenCalledWith([
    expect.objectContaining({
      kind: 'highlight',
      text: 'A passage worth remembering',
      cfi: 'epubcfi(/6/2!/4/2:0)',
    }),
  ]);
  expect(ctx.view.addAnnotation).toHaveBeenCalledWith({
...
it('restores saved highlights when opening and when an overlay is recreated', async () => {
  ...
  await act(async () => {
    ctx.view.dispatchEvent(
      new CustomEvent('create-overlay', { detail: { index: 0 } }),
    );
  });
  expect(ctx.view.addAnnotation).toHaveBeenCalledTimes(2);
  await click('Bookmarks and highlights');
  expect(document.body.textContent).toContain('Saved');
  await click('Delete saved passage');
  expect(ctx.onSave).toHaveBeenCalledWith([]);
  expect(ctx.view.deleteAnnotation).toHaveBeenCalledWith({
    value: 'epubcfi(/6/2)',
  });
  await act(async () => ctx.root.unmount());
});
...
it('keeps the note draft through keyboard reflow and saves its text', async () => {
  ...
  await act(async () => {
    Object.getOwnPropertyDescriptor(
      ...
      'value',
    )!.set!.call(textarea, 'Remember this detail');
    textarea.dispatchEvent(new Event('input', { bubbles: true }));
  });
  await act(async () => {
    ctx.view.dispatchEvent(
      new CustomEvent('relocate', { detail: { cfi: 'reflowed-location' } }),
    );
  });
...
it('shows API definitions as themed text with source attribution', async () => {
  vi.stubGlobal(
    ...
    vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        parse: {
          text: '<h2>English</h2><h3>Noun</h3><ol><li>The first letter.</li></ol>',
        },
      }),
    }),
  );
  ...
  await click('Define selected text');
  expect(document.querySelector('iframe')).toBeNull();
  expect(document.body.textContent).toContain('The first letter.');

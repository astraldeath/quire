...
it('shrinks the dialog scroll area instead of recentering the header above its original position',async()=>{
 ...
 vi.stubGlobal('ResizeObserver',class{constructor(callback:()=>void){resizeContent=callback;}observe(){}disconnect(){}});
 ...
 try{
  ...
  vi.mocked(dialog.getBoundingClientRect).mockReturnValue({height:740} as DOMRect);
  await act(async()=>resizeContent!());
  expect(dialog.style.top).toBe('52px');
  vi.mocked(dialog.getBoundingClientRect).mockReturnValue({height:680} as DOMRect);
  await act(async()=>resizeContent!());
 ...
 }finally{await act(async()=>root.unmount());host.remove();vi.restoreAllMocks();vi.unstubAllGlobals();if(showModal)Object.defineProperty(HTMLDialogElement.prototype,'showModal',showModal);else Reflect.deleteProperty(HTMLDialogElement.prototype,'showModal');if(previous)Object.defineProperty(window,'visualViewport',previous);else Reflect.deleteProperty(window,'visualViewport');}
});
